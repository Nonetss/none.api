...
export async function mapDependencies(url: string, args: any) {
  ...
  result.providers.forEach((p: any) => text += `- ${p.method} ${p.path} (Provides: ${p.provides})\n`);
  text += "\n### Data Consumers (Endpoints that require IDs):\n";
  result.consumers.forEach((c: any) => text += `- ${c.method} ${c.path} (Consumes: ${c.consumes} in ${c.in})\n`);
